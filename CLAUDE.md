...
- `docker-compose.yml` — seven application services: `shell` (5173, dev mode for HMR), `mfe-client`/`mfe-ops`/`mfe-admin` (5181/5182/5183, **build + `vite preview`** because federation expose only works after build), `bff-client`/`bff-ops`/`bff-admin` (8081/8082/8083), plus Keycloak (8888), `user-service` (8090), Postgres. Every frontend container mounts the same `./frontend` to `/workspace`, runs `npm install` against the root workspace, then runs the script for *its* workspace only. Federation remote URLs live in env vars on the `shell` service (`VITE_MFE_*_URL`) — these are baked into the shell bundle at config-time and must be **browser-reachable** (so `localhost:518X`, not `mfe-client:5181`). BFF URLs on the same service (`BFF_*_URL`) drive the in-network Vite proxy, so they use compose hostnames.
