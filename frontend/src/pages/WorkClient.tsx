...
export function WorkClient() {
  ...
  return (
    <div className="work">
      <section>
        <div className="section-eyebrow">Account &amp; support</div>
        <h2 className="section-title">What can we help with today?</h2>
        <p className="section-body">
          Drop us a note, check on recent orders, or kick off a new request. Nothing on this tab
          posts for real — it's a mock workflow so you can feel the shape of the client experience.
        </p>
      </section>
      ...
      <section className="work-grid">
        ...
        <div className="card card-wide">
          ...
          <div className="button-row">
            <button
              onClick={() => setActionsNotice('Callback scheduled for tomorrow at 10:00 local time.')}
            >
              Schedule a callback
            </button>
          ...
          <div className={`demo-notice${actionsNotice ? ' visible' : ''}`}>{actionsNotice ?? ''}</div>
        </div>
      </section>
    </div>
  );
